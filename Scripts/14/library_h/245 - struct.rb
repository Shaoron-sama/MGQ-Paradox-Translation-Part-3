...
module LibraryH
  class << self
    def init_background(background)
      if background.nil?
        return { :pic => NWConst::Library::H_SCENE_DEFAULT_MEMORY_BG_IMAGE }
      end
  ...
  class Item
    ...
    attr_reader :name, :common, :map, :condition, :fadeout
  ...
  class Cg
    ...
    def condition
      @_condition ||= Condition.make(@condition)
    end
  ...
  class Charactor
    ...
    attr_reader :key, :name, :condition, :cgs, :items
