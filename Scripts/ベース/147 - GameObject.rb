class Game_Action
  ...
  def check_chain_action(data = nil)
    return if @chain_action.nil? || @subject.nil? || object.nil?
...
class Game_ActionResult
  ...
  def restoration_text(subject)
    ary = []
    ary.push("HP #{@hp_restore.give_unit_floor(12)} ") if 0 < @hp_restore
    ary.push("MP #{@mp_restore.give_unit_floor(12)} ") if 0 < @mp_restore
    format(Vocab::ReStoration, subject.name, ary.join("、"))
  end
  ...
  def hp_damage_text
    if @hp_drain > 0
      ...
      value = MAX_DAMAGE if @hp_drain >= (10**72)
      format(fmt, @battler.name, Vocab.hp, value)
    elsif @hp_damage > 0
      ...
      value = MAX_DAMAGE if @hp_damage >= (10**72)
...
class Game_Actor < Game_Battler
  ...
  def repeat_action
    ...
    @actions.map.with_index do |_, i|
      ...
      if action.nil? || (!action.attack? && !s.include?(action.object)) || !action.valid? || bind?
        auto ||= auto_battle_actions(actions.size, :normal)
        next auto[i]
      end
      action.check_chain_action
      action.clear_speed
      action
    end
  end
  ...
  def set_action_history
    @action_history = @actions.select(&:object)
  end
...
class Game_Battler < Game_BattlerBase
  ...
  def make_action_times
    action_plus_set.inject(1) do |r, p|
      par = p
      if par > 1.0
        r += par / 1.0
        par %= 1.0
  ...
  def item_final_elements_raw(item, include_atk_elements = true)
    ...
    (all_add_elements + item_elements(item, include_atk_elements)).uniq.map do |element_id|
...
class Game_Enemy < Game_Battler
  ...
  def param_calc_variable(value, rate_var_id, plus_var_id)
    ((value + param_plus_variable(plus_var_id)) * param_rate_variable(rate_var_id)).to_i
  end
...
class Game_Party < Game_Unit
  ...
  def restore_socket_item(item)
    ...
    return item unless item_number(item) > 0 && item.stones.compact.empty?

    delete_item_data(item)
    gain_item(item.base_data, 1)
    item.base_data
  end
  ...
  def restore_uniq_items
    ...
    [@enchant_weapons, @enchant_armors, @socket_weapons, @socket_armors].each do |c|
      c.item_data do |data|
        next unless data.item_number == 0 && data.storehouse_item_number == 0 && data.item_object.equip_actor.nil?
