...
module BattleManager
  class << self
    ...
    #--------------------------------------------------------------------------
    def no_lose_skip?
      enemy_id = $game_temp.lose_event_id - NWConst::Common::LOSE_EVENT_BASE
    ...
    def gain_drop_items
      ...
      $game_troop.make_drop_items.each do |item|
        ...
        $game_party.gain_item(item.replace_data, 1)
      end

      $game_temp.get_item_data.each do |item, value|
        next $game_message.add(Vocab.item_get_message(item, 0)) if value == 0
    ...
    def process_victory
      @phase = nil
      SceneManager.scene.process_event_on_battle_end(:victory)
    ...
    def process_defeat
      @phase = nil
      SceneManager.scene.process_event_on_battle_end(:defeat) if $game_temp.common_event_reserved?
    ...
    def change_novel_scene
      ...
      $game_novel.setup($game_temp.lose_event_id)
    ...
    def process_follow_choice(follower_name = nil)
      follower_name ||= $game_troop.follower_enemy.enemy.base_enemy.name
    ...
    def make_action_orders
      ...
      @fastest_actions = [@action_battlers.first]
      @fastest_actions << @fastest_actions.last.chain_action while @fastest_actions.last.chain_action
    end
    ...
    def mimic_skills(battler)
      m = mimic_items.last
      return [] if m.nil? || battler.mimic_usable?(m)
    ...
    def fastest_action?(action)
      ...
      @fastest_actions.include?(action)
    end
...
module Cache
  class << self
    ...
    def face(filename, hue = 0)
      load_bitmap("Graphics/Faces/", filename, hue)
    end
  ...
  class << self
    ...
    def latest_updatetime
      ...
      files = ["Data/Actors", "Data/Classes", "Data/Skills", "Data/Items", "Data/Weapons", "Data/Armors",
               "Data/Enemies", "Data/Troops", "Data/States", "Data/Animations", "Data/Tilesets", "Data/CommonEvents", "Data/System", "Data/MapInfos"]
      i = 1
      while File.exist?("#{over_map_dir(i)}")
        files << "#{over_map_dir}/Data/MapInfos"
        i += 1
      end
    ...
    def nw_analyze_database
      ...
      $data_common_events.compact.each do |ev|
        while ev.list.first && ev.list.first.code == 999
          ev.list.shift
        end
        ev.list.unshift(RPG::EventCommand.new(999, 0, [ev.id])) 
      end
    ...
    def make_thumbnail
      ...
      Dir.entries("Save").each do |file_name|
        ...
        elsif NWFileTest.system_exist?("NotThumbnailFile.png")
          @thumbnails[Regexp.last_match(1).to_i - 1] =
            Bitmap.new("Graphics/System/NotThumbnailFile.png")
        end
      end
    end
    ...
    def update_system
      ...
      $game_library.refresh_open_stone
    end

    def over_map_dir(dir_id = 1)
      d = dir_id == 1 ? "" : dir_id.to_s
      "Data/Map#{d}"
    end
    ...
    def map_file_name(map_id)
      ...
      format("#{over_map_dir(div)}/Data/Map%03d.rvdata2", mod)
    end
  end
end
...
module Help
  class << self
    ...
    #--------------------------------------------------------------------------
    def equip_info_change
      "←/→,#{Vocab.key_l}/#{Vocab.key_r}:情報ページ切替"
    end
...
module Vocab
  ...
  NotObtainItem = "%sはこれ以上持てないようだ…"
  ACTOR_DAMAGE = "%sは %s の%sダメージを受けた！"
  ENEMY_DAMAGE = "%sに %s の%sダメージを与えた！"
  STATE_BOOST = "状態異常特攻！！"
  EX_CATEGORY_BOOST = "種族特攻！！"
  ...
  PartySaveMessage = "どのスロットにパーティを登録しますか？"
  PartyLoadMessage = "どのパーティを呼び出しますか？"
  ...
  class << self
    ...
    def params_a(param_id)
      ["攻撃", "防御", "魔力", "精神", "素早", "器用","ＨＰ","ＭＰ"][param_id]
    end

    def ex_category(category)
      EX_CATEGORY.fetch(category, nil)
    end

    def etype(etype_id)
      ($data_system.terms.etypes + ["能力アクセ", "通常アクセ"])[etype_id]
    end
    ...
    def item_get_message(item, value, is_display_value = false)
      ...
      if value == 0
        ...
        format(ObtainItem, message + (item.enchant_item? || !is_display_value ? "" : " #{value}個"))
      end
    end
  end
end
