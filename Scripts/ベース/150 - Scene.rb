...
class Scene_ActorSelectBase < Scene_MenuBase
  ACTOR_SELECT_HELP_MESSAGE = "キャラクターを選択してください"
  def start
    super
    create_all_window
...
class Scene_Battle < Scene_Base
  ...
  COST_NOT_PAY_MESSAGE = "消費%sが足りないため、ターン中に回復しないと発動できません。\\n選択スキルを使用しますか？"
  ...
  def display_skill_name(item, action = nil)
    ...
    $game_party.display_skill_name = item.visible? ? item.display_name : nil
  end
      ...
      use_items.each_with_index do |item, item_time|
        ...
        if enable_invoke # 効果の発動
          ...
          targets.each do |target|
            hit_count[target] ||= 0
            hit_count[target] += 1
            item.repeats.times { invoke_item(target, item, :hit_count => hit_count, :base_item => base_item) }
          end
        ...
        if enable_invoke && enable_counter # 通常反撃
          targets.uniq.each do |target|
            ...
            invoke_counter_attack(target, item, $data_skills[counter_skill]) if counter_skill && $data_skills[counter_skill]
          end
        end
  ...
  def use_item_learning_skill(item)
    ...
    texts = mem.map do |member|
      "#{member.name}は #{$data_skills[item.learning_skill].window_name} をラーニングした！"
    end
...
class Scene_EquipStoneActor < Scene_EquipStoneBase
  ...
  def show_key_text
    ...
    if @stones_window.active
      ...
      [ShowKey_Help.stone_change_scene, ShowKey_Help.lr_actor]
    else
      [ShowKey_Help.stone_change_scene, ShowKey_Help.stone_shift, ShowKey_Help.equip_info, ShowKey_Help.lr_actor]
    end
  end
...
class Scene_EquipStoneItem < Scene_EquipStoneBase
  ...
  def show_key_text
    ...
    if @stones_window.active
      ...
      [ShowKey_Help.stone_shift, ShowKey_Help.equip_info]
    end
  end
