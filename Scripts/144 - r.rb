module NWRegexp
  ...
  FEATURE_XPARAM_EX_ARRAY = %w[命中 回避 会心 会心回避 魔法回避 魔法反射 反撃 HP再生 MP再生 TP再生]
  ...
  EX_VALUES = %w[戦闘回数 倒した数 イかせた数 倒された数 イかされた回数 盗んだ回数]
  ...
  module BaseItem
    ...
    CERTAIN_EVASION = /<(拡張|)必中回避(率|)\s?(#{VALUE}%)>/i
    ...
    AUTO_SKILL_INVALID = /(#{INVALID_ARRAY.join("|")})無効\s?((?:\d+(?:,\s?)?)+)/i
    ...
    REGENERATION_RATE_FIX = /<(HP|MP|TP)再生(?:率|)固定 (#{VALUE})%>/
