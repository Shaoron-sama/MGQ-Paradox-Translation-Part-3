...
class Game_Interpreter
  ...
  def change_friend(value)
    ...
    text = format("%sの友好度が%d%s", enemy.name, value.abs, value > 0 ? "上がった！" : "下がった！")
    enemy.friend += value
    se.play
  ...
  def reset_actor(actor_id)
    ...
    $game_actors[actor_id].clear_equipments
    $game_actors[actor_id].setup(oid)
    $game_actors[actor_id].persona_change(actor_id)
  end
