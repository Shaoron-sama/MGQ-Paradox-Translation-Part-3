...
module RPG
  module Uniq_Item
    ...
    def include_item_list
      return false unless change_ok? 
      return true if equip_actor.nil?

      $game_party.include_members.include?(equip_actor)
    end

    def initialize(item)
      @base_id = item.id
      @id = $game_party.uniq_item_number(item)
    end
