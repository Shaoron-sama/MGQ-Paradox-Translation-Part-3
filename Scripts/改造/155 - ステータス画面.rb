...
module NWConst::Status
  ...
  BASIC_PARAM_VOCAB     = [
    ...
    "命中率", "会心率","魔法会心率","会心ダメージ率","回避率", "魔法回避率","万能回避率", "カウンター率", "魔法カウンター率","万能カウンター率"
  ]
  # 特殊能力
  SPECIAL_PARAM_VOCAB   = [
    "HP再生率", "MP再生率", "SP再生率", "SPチャージ率", "MP消費率", "SP消費率",
    "回復効果率", "アイテム効果率", "盗み成功率", "反射率", "魔法反射率", "万能反射率",
    "経験獲得率", "職業経験獲得率", "獲得金額倍率", "獲得アイテム倍率" ,"エンカウント率"
  ]
  # 属性対応アイコンID
  ELEMENT_ICONS         = [244,188,144,145,146,149,148,147,150,151,51,176]
  ADD_ELEMENT_ICONS     = ELEMENT_ICONS[0..-3].flat_map{|e|[e,e]} + ELEMENT_ICONS[-2..-1] + [157]
...
#~       (1..5).collect{|i| "装備品#{i}"},
      @actor ? @actor.basic_equip_slots.map{|i| "#{Vocab.etype(i)}情報"} : [],
