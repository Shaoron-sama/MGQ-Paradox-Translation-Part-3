module MultiParty
  ...
  class Window_MultiMember < Window_Command
    ...
    def initialize(x, y, number, fix_actors)
      @member_size = 1
      @number = number
    ...
    def make_command_list
      add_command("パーティ#{@number}", :n, false)
  ...
  class Windows_MultiMember < BasicObject
    ...
    def refresh
      ...
      @confirm_window.move(@data.first.x,@data.last.y + @data.last.height, @data.first.window_width, @confirm_window.window_height)
    end
