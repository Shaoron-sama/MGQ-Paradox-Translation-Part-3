class Scene_MultiPartyEdit < Scene_MenuBase
  ERROR_MESSAGE = "いずれかのパーティに %s を編成してください。"
  CONFIRM_MESSAGE = "パーティ編成を完了しますか？"
  CANCEL_MESSAGE = "パーティ編成をキャンセルしますか？"
  ...
  def show_key_text
    ...
    result = @wait_member_window.active ? [] : ["#{Vocab.key_a}:外す"]
    result += ["#{Vocab.key_x}:ステータス", "#{Vocab.key_y}: ソート"]
    result
  end
  ...
  def create_party_member_window
    ...
    @party_size.times do |i|
      w = MultiParty::Window_MultiMember.new(0, l, i + 1, @fix_members[i])
      ws.push(w)
