...
class RPG::BaseItem
  ...
  def nw_line_analyze(line)
    ...
    when NWRegexp::BaseItem::CERTAIN_EVASION
      add_feature(FEATURE_BATTLER_ABILITY, CERTAIN_EVASION, get_value(Regexp.last_match(3), :rate))
    ...
    when NWRegexp::BaseItem::AUTO_SKILL_INVALID
      ...
      Regexp.last_match(2).split(/,\s?/).each do |s|
        add_feature(FEATURE_AUTO_SKILL_INVALID, index, s.to_i)
      end
    ...
    when NWRegexp::BaseItem::REGENERATION_RATE_FIX
      id = ["HP","MP","TP"].index(Regexp.last_match(1)) + 7
      add_feature(FEATURE_XPARAM_FIX, id, get_value(Regexp.last_match(2), :rate))
